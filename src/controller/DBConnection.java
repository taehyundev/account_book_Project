...
public class DBConnection {
	...
	public boolean createTranTable(String userId) {
		...
		try {
			...
			sql = sb.append("create table if not exists user_"+userId+"(")
					...
					.append("date TIMESTAMP DEFAULT NOW()")
					.append(");").toString();
			st.execute(sql);
			return true;
		}catch (Exception e) {
	...
	public List<Transaction> getTranTable(String userId) {
		String sql ="";
		try {
			...
			while(rs.next()) {
				...
				t = tb.name(rs.getString("name")).type(rs.getString("type")).amount(Double.parseDouble(rs.getString("amount"))).note(rs.getString("memo")).date(rs.getString("date")).transaction();
